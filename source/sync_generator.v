...
//
//功能介绍：
//模块`sync_generator`用于生成标准视频信号的同步控制信号和活动显示区域的像素坐标。
//它通过水平和垂直计数器来控制水平和垂直同步信号的时序，符合1080p视频格式。
//水平同步信号`hs_out`和垂直同步信号`vs_out`按H_SYNC和V_SYNC时长输出，并根据视频帧的显示周期更新数据有效信号`de_out`和坐标`x_act`、`y_act`。
//其中，`de_out`标记数据有效区域，`de_re`为调节后的数据有效信号。

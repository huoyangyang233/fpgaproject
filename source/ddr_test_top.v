...
//## 模块概述
//
//`test_ddr` 模块是一个综合性设计，集成了DDR3内存接口、HDMI输入输出、IIC通信、CMOS摄像头接口、PCIe接口及以太网（ETH）接口等多个功能模块。
//该模块主要用于处理高清视频信号、图像数据存储和传输，并支持通过PCIe和以太网进行数据交换。
//
//## 输入端口
//
//### 时钟与复位
//- **ref_clk** (`input`): 参考时钟信号。
//- **rst_board** (`input`): 全局复位信号，低有效。
//- **pix_clk_in** (`input`): HDMI输入的像素时钟，1080p @148.5MHz。
//- **cmos1_pclk** (`input`): CMOS1摄像头的像素时钟。
//- **cmos2_pclk** (`input`): CMOS2摄像头的像素时钟。
//- **pcie_perst_n** (`input`): PCIe复位引脚，低有效。
//
//### HDMI输入
//- **vs_in** (`input`): HDMI输入的垂直同步信号。
//- **hs_in** (`input`): HDMI输入的水平同步信号。
//- **de_in** (`input`): HDMI输入的数据有效信号。
//- **r_in** (`input [7:0]`): HDMI输入的红色分量数据。
//- **g_in** (`input [7:0]`): HDMI输入的绿色分量数据。
//- **b_in** (`input [7:0]`): HDMI输入的蓝色分量数据。
//
//### CMOS摄像头接口
//- **cmos1_scl** (`inout`): CMOS1摄像头的I2C时钟线。
//- **cmos1_sda** (`inout`): CMOS1摄像头的I2C数据线。
//- **cmos1_vsync** (`input`): CMOS1摄像头的垂直同步信号。
//- **cmos1_href** (`input`): CMOS1摄像头的行同步/数据有效信号。
//- **cmos1_data** (`input [7:0]`): CMOS1摄像头的数据。
//- **cmos2_scl** (`inout`): CMOS2摄像头的I2C时钟线。
//- **cmos2_sda** (`inout`): CMOS2摄像头的I2C数据线。
//- **cmos2_vsync** (`input`): CMOS2摄像头的垂直同步信号。
//- **cmos2_href** (`input`): CMOS2摄像头的行同步/数据有效信号。
//- **cmos2_data** (`input [7:0]`): CMOS2摄像头的数据。
//
//### PCIe接口
//- **rxn** (`input [3:0]`): PCIe接收负向差分信号。
//- **rxp** (`input [3:0]`): PCIe接收正向差分信号。
//- **txn** (`output [3:0]`): PCIe发送负向差分信号。
//- **txp** (`output [3:0]`): PCIe发送正向差分信号。
//
//### 以太网接口
//- **eth_rgmii_rxc_0** (`input`): ETH0 RGMII接收时钟。
//- **eth_rgmii_rx_ctl_0** (`input`): ETH0 RGMII接收控制信号。
//- **eth_rgmii_rxd_0** (`input [3:0]`): ETH0 RGMII接收数据。
//- **eth_rgmii_txc_0** (`output`): ETH0 RGMII发送时钟。
//- **eth_rgmii_tx_ctl_0** (`output`): ETH0 RGMII发送控制信号。
//- **eth_rgmii_txd_0** (`output [3:0]`): ETH0 RGMII发送数据。
//
//## 输出端口
//
//### DDR3内存接口
//- **ddr_pll_lock** (`output`): DDR PLL锁定信号。
//- **ddr_init_done** (`output`): DDR初始化完成信号。
//- **mem_rst_n** (`output`): 内存复位信号，低有效。
//- **mem_ck** (`output`): 内存时钟正向。
//- **mem_ck_n** (`output`): 内存时钟负向。
//- **mem_cke** (`output`): 内存时钟使能。
//- **mem_cs_n** (`output`): 内存片选信号，低有效。
//- **mem_ras_n** (`output`): 内存RAS信号，低有效。
//- **mem_cas_n** (`output`): 内存CAS信号，低有效。
//- **mem_we_n** (`output`): 内存写使能信号，低有效。
//- **mem_odt** (`output`): 内存ODT信号。
//- **mem_a** (`output [MEM_ROW_ADDR_WIDTH-1:0]`): 内存地址总线。
//- **mem_ba** (`output [MEM_BADDR_WIDTH-1:0]`): 内存银行地址。
//- **mem_dm** (`output [MEM_DM_WIDTH-1:0]`): 内存掩码信号。
//- **mem_dqs** (`inout [MEM_DQS_WIDTH-1:0]`): 内存DQS信号。
//- **mem_dqs_n** (`inout [MEM_DQS_WIDTH-1:0]`): 内存DQS负向信号。
//- **mem_dq** (`inout [MEM_DQ_WIDTH-1:0]`): 内存数据总线。
//
//### HDMI输出
//- **pix_clk_out** (`output`): HDMI输出的像素时钟。
//- **r_vs_out** (`output reg`): HDMI输出的垂直同步信号。
//- **r_hs_out** (`output reg`): HDMI输出的水平同步信号。
//- **r_de_out** (`output reg`): HDMI输出的数据有效信号。
//- **r_r_out** (`output reg [7:0]`): HDMI输出的红色分量数据。
//- **r_g_out** (`output reg [7:0]`): HDMI输出的绿色分量数据。
//- **r_b_out** (`output reg [7:0]`): HDMI输出的蓝色分量数据。
//
//### IIC接口
//- **iic_tx_scl** (`output`): IIC传输时钟线。
//- **iic_tx_sda** (`inout`): IIC传输数据线。
//- **iic_scl** (`output`): IIC时钟线。
//- **iic_sda** (`inout`): IIC数据线。
//
//### 其他输出
//- **hdmi_rst** (`output wire`): HDMI复位信号。
//- **hdmi_int_led** (`output wire`): HDMI指示灯。
//- **fram0_done** 至 **fram3_done** (`output wire`): 各视频帧完成信号。
//- **cmos1_reset** (`output`): CMOS1摄像头复位信号。
//- **cmos2_reset** (`output`): CMOS2摄像头复位信号。
//- **eth_rst_n_0** (`output wire`): 以太网复位信号。
//- **eth_rgmii_txc_0** 等：以太网RGMII发送信号。
//
//## 模块功能
//
//### 1. DDR3内存接口
//- **初始化与控制**：通过DDR3 PLL进行时钟生成和锁定，完成内存初始化后，提供`ddr_init_done`信号。
//- **AXI接口**：集成AXI写读通道，通过`axi_m_arbitration`子模块管理AXI事务，实现与DDR3内存的数据交换。
//
//### 2. HDMI输入输出
//- **HDMI输入处理**：接收高清视频信号（1080p），通过同步生成器和视频处理模块（如缩放、增强）处理视频数据。
//- **HDMI输出生成**：将处理后的高清视频信号通过HDMI接口输出，支持同步信号和RGB数据的传输。
//
//### 3. IIC通信
//- **HDMI控制**：通过IIC接口配置HDMI控制器，实现HDMI的初始化和配置。
//- **摄像头配置**：通过IIC接口配置CMOS摄像头（OV5640），完成摄像头的初始化。
//
//### 4. CMOS摄像头接口
//- **数据采集与处理**：接收来自两路CMOS摄像头的数据，进行8位到16位的数据转换，并通过FIFO缓存后传输至DDR3内存或视频处理模块。
//- **同步信号处理**：处理摄像头的垂直同步和行同步信号，确保数据的正确采集和传输。
//
//### 5. PCIe接口
//- **数据传输**：通过PCIe接口实现高速数据传输，支持AXIS主从接口进行数据交换。
//- **DMA控制**：集成PCIe DMA控制模块，管理数据的读写请求和传输完成信号。
//
//### 6. 以太网接口
//- **数据封装与解封**：将视频数据封装为IP帧格式，通过以太网进行传输，并能接收和处理来自以太网的数据包。
//- **UDP通信**：集成UDP协议栈，实现视频数据的网络传输和接收。
//
//### 7. 视频处理
//- **视频缩放**：通过`video_zoom`子模块实现视频信号的缩放处理，支持输入视频的分辨率调整。
//- **视频增强**：通过`video_enhance`子模块对视频信号进行增强处理，包括亮度和对比度调整。
//
//### 8. 系统复位与时钟同步
//- **复位同步**：使用同步复位模块确保不同时钟域下的复位信号同步，避免时序问题。
//- **时钟生成**：通过PLL模块生成所需的各种时钟信号，确保各功能模块在正确的时钟下运行。
//
//### 9. 调试与监控
//- **调试信号**：提供多种调试信号和状态指示灯，如`hdmi_int_led`、`fram*_done`等，便于系统状态监控和调试。
//- **状态计数**：通过计数器和状态机监控视频信号的帧同步和数据有效性，确保数据传输的准确性。
//
//## 总结
//
//`test_ddr` 模块集成了多种外设接口和功能模块，主要用于高清视频信号的采集、处理、存储和传输。通过与DDR3内存的交互，支持大容量的视频数据存储；
//通过PCIe和以太网接口，实现高速的数据传输和网络通信；同时，集成的视频处理模块提升了视频信号的质量和适应性。
//该模块适用于需要高性能视频处理和数据传输的应用场景，如视频监控系统、图像处理设备和高带宽数据传输系统。
//
`timescale 1ps/1ps
...
parameter  DES_MAC   = 48'h58_11_22_81_01_70;
...
eth_img_pkt eth0_img_pkt(    
    ...
    .img_data_en        (eth_zoom_de_out     ), //数据有效使能信号              
    .img_data           ({eth_zoom_data_out[31 : 27],eth_zoom_data_out[21 : 16],eth_zoom_data_out[11 :  7]}), //input  [15:0]   //有效数据
    .transfer_flag      (1              ), //图像开始传输标志,1:开始传输 0:停止传输                                     
    ////以太网相关信号

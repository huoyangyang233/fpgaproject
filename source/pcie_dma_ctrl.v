//模块 `pcie_dma_ctrl` 主要用于在 PCIe 和 HDMI 之间实现数据传输控制，特别是通过 DMA（Direct Memory Access）实现数据的高速传输。本模块具备视频增强的功能控制，可以控制暗光增强和高光压制，同时支持不同的 DMA 命令。
//
//### 输入端口
//- **主时钟和复位**：
//  - `clk`：主时钟信号。
...
//  - `rstn`：复位信号，低电平复位。
//  
//- **AXI-Stream 接口**（用于接收 DMA 控制信息）：
//  - `axis_master_tvalid`：主机数据有效信号。
//  - `axis_master_tready`：主机数据准备信号。
//  - `axis_master_tdata`：主机数据总线（128位）。
//  - `axis_master_tkeep`：主机数据有效字节指示（4位）。
//  - `axis_master_tlast`：主机数据结束标志。
//  - `axis_master_tuser`：主机用户标识。
//
//- **PCIe 控制信号**：
//  - `ep_bus_num` 和 `ep_dev_num`：表示 PCIe 端点的总线和设备编号。
//
//- **视频同步信号**：
//  - `vs_in`、`de_in`：垂直同步和数据有效信号。
//
//- **视频增强控制**：
//  - `video_enhance_lightdown_num`：高光压制值。
//  - `video_enhance_lightdown_sw`：高光压制开关。
//  - `video_enhance_darkup_num`：暗光增强值。
//  - `video_enhance_darkup_sw`：暗光增强开关。
//
//### 输出端口
//- **AXI-Stream 接口**：
//  - `AXIS_S_TVALID`：AXI 从机数据有效信号。
//  - `AXIS_S_TREADY`：AXI 从机数据准备信号。
//  - `AXIS_S_TDATA`：AXI 从机数据总线（128位）。
//  - `AXIS_S_TLAST`：AXI 从机数据结束标志。
//  - `AXIS_S_TUSER`：AXI 从机用户标识。
//
//### 模块内部逻辑和工作原理
//1. **输入数据寄存**：对 AXI-Stream 数据接口的输入数据进行打拍，存储到寄存器中，以稳定数据流。
//
//2. **解析 TLP 格式**：根据接收到的数据包类型 `tlp_fmt` 和 `tlp_type`，判断接收数据的类型以及执行的操作。
//
//3. **DMA 地址管理**：通过 `alloc_addr_state` 控制分配 DMA 地址，每次执行 DMA 传输时从 `dma_addr0` 到 `dma_addr3` 循环选择地址。
//
//4. **视频增强控制**：当接收到指定的 DMA 命令时，可以控制视频增强的高光压制和暗光增强功能，通过 `video_enhance_lightdown_num` 和 `video_enhance_darkup_num` 配置增强程度。
//
//5. **状态机控制 DMA 传输**：`mwr_state` 状态机负责 AXI-Stream 数据传输，按顺序发送 TLP 包头和包数据，实现完整的数据流传输。每个 TLP 包数据长度为 `TLP_LENGTH`。
//
//6. **读写控制**：当 `vs_in` 下降沿时，表示开始一个新的数据帧，此时 `fram_start` 信号拉高，表明开始传输。通过对 FIFO 的水位监控控制 `pcie_rd_en` 信号，确保在 FIFO 中有足够数据时才开始 DMA 传输。
//
//7. **视频数据 FIFO 缓存**：`hdmi_pcie_fifo` 用于存储 HDMI 输入数据，并在需要时将数据传输至 PCIe。
//
//### 实现功能
//- 支持不同类型的 DMA 命令，包括视频增强控制、高光压制、暗光增强、DMA 地址清除等。
//- 实现从 HDMI 到 PCIe 的数据流传输。
//- 对视频信号进行增强控制，通过调整亮度和对比度改善图像质量。

//该 `eth_img_pkt` 模块实现了图像数据的以太网UDP传输，具体步骤包括数据帧头的生成、图像数据打包、UDP传输控制以及发送帧间隔管理。模块的功能是将摄像头图像数据以帧为单位进行打包，通过UDP协议发送至以太网接收端。
//
//### 主要功能模块
//
//1. **参数设置**:
//   - `CMOS_H_PIXEL` 和 `CMOS_V_PIXEL`：设置图像分辨率为960x540。
//   - `UDP_DATA_SIZE`：每个UDP包的有效数据长度为960字节。
//   - `IMG_FRAME_HEAD`：图像帧头，用于指示一帧数据的起始。
//
//2. **输入信号**:
//   - `cam_pclk`：像素时钟。
//   - `img_vsync`：帧同步信号，用于区分帧起始。
//   - `img_data_en`：数据有效信号，表示有效图像数据。
//   - `img_data`：16位图像数据。
//   - `transfer_flag`：传输使能信号，用于控制是否开始图像数据的传输。
//   - `eth_tx_clk`：以太网发送时钟。
//   - `udp_tx_req`：UDP发送请求信号。
//   - `udp_tx_done`：UDP数据发送完成信号。
//
//3. **输出信号**:
//   - `udp_tx_start_en`：UDP发送开始信号，控制数据发送。
//   - `udp_tx_data`：待发送的32位数据。
//   - `udp_tx_byte_num`：每个UDP包的字节数。
//
//### 操作流程
//
//模块通过多个状态和计数器管理数据的读取、打包和发送流程，核心流程如下：
//
//1. **图像数据帧头生成和图像数据打包**：
//   - 进入一帧的起始时，首先写入帧头`IMG_FRAME_HEAD`，然后写入图像的分辨率信息。
//   - 将每两个16位的图像数据打包为一个32位数据写入FIFO，实现数据拼接与缓存。
//
//2. **UDP包控制**：
//   - 每个UDP包包含960字节图像数据和相应的帧头或标识。
//   - `img_pkt_cnt` 作为数据包计数器，标记当前UDP包在帧中的顺序。
//   - 当接收完整一帧的数据后，`neg_vsync`控制信号使图像数据缓存清空，以备下一帧图像数据的传输。
//
//3. **以太网发送控制**：
//   - `tx_busy_flag`标识当前是否处于数据发送状态，`udp_tx_start_en`控制每个UDP包的发送开始信号。
//   - `fifo_rdusedw`表示当前FIFO中的有效数据量，当其达到预定字节数时，启动一次UDP传输。
//   - `udp_tx_byte_num`设置每个UDP包的有效数据长度，包括帧头、分辨率等控制信息。
//
//4. **发送帧间隔管理**：
//   - `eth_delay_cnt`计数器确保每帧发送之间的最小间隔，以满足以太网的帧间隔要求。
//
//5. **异步FIFO**：
//   - 通过异步FIFO `eth_pkt_fifo` 实现像素时钟域和以太网发送时钟域之间的数据缓冲与同步。
//
